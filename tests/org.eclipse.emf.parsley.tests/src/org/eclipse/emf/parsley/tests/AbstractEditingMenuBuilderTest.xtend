...
abstract class AbstractEditingMenuBuilderTest extends AbstractEmfParsleyTest {
	 ...
	 */
	static class TestableEditingMenuBuilder extends EditingMenuBuilder {
		override getDeleteAction() {
			return super.deleteAction;
		}

		override getCutAction() {
			return super.cutAction;
		}

		override getCopyAction() {
			return super.copyAction;
		}

		override getPasteAction() {
			return super.pasteAction;
		}

		override getUndoAction() {
			return super.undoAction;
		}

		override getRedoAction() {
			return super.redoAction;
		}

		override createLoadResourceAction() {
			return super.createLoadResourceAction;
		}

		override createControlAction() {
	...
	@Test
	def void testMenuWithNotEObject() {
	...
	def void testDefaultMenu() {
		val editingMenuBuilder = getAndInitializeEditingMenuBuilder
		editingMenuBuilder.assertMenuItemsGivenObject(library,
		"&Undo @Ctrl+Z, &Redo @Ctrl+Y, separator, Cu&t, &Copy, &Paste, separator, &Delete, separator")
	}
...
'''Stock Book, Stock Book On Tape, Stock Video Cassette, Branches Library, Writers Writer, Employees Employee, Borrowers Borrower'''
	...
	def void testCustomMenu() {
		val editingMenuBuilder = new EditingMenuBuilder() {
			...
			def protected menuContributions(Library o) {
				#[
					actionRedo(),
					actionUndo(),
					separator(),
...
, &Paste'''
	...
	def void testCustomAddCommand_Bug466219() {
		...
		val resource = createResourceForTest
	...
	@Test
	def void testActionsAreCreatedByCreateActions() {
		createTestableEditingMenuBuilder => [
			collectActions.forEach[assertNotNull]
		]
	}

	@Test
	def void testSafeActionUpdate() {
		createTestableEditingMenuBuilder => [
			updateEditingDomain(getEditingDomain)
			updateUndoRedo
		]
	}

	@Test
	def void testCreateOtherNonDefaultActions() {
		createTestableEditingMenuBuilder => [
			createLoadResourceAction.assertNotNull
			createControlAction.assertNotNull
			createValidateAction.assertNotNull
		]
	}
	...
	def protected getAndInitializeEditingMenuBuilder() {
	...
	def private createTestableEditingMenuBuilder() {
		new TestableEditingMenuBuilder().injectMembers => [
			initializeEditingMenuBuilder()
		]
	}
	...
	def protected assertMenuItemsGivenObject(EditingMenuBuilder editingMenuBuilder, Object o, CharSequence expectedRepresentation) {
	...
	def protected commandStackListener(CommandStackListener listener) {
	 ...
	 */
	def protected oneTimeCommandStackListener(CommandStackListener listener) {
		oneTimeCommandStackListener(listener, editingDomain)
	}

	def protected commandStackListener(CommandStackListener listener, EditingDomain editingDomain) {
	 ...
	 * The listener will get only one event.
	 */
	def protected oneTimeCommandStackListener(CommandStackListener listener, EditingDomain editingDomain) {
	...
	def private collectActions(TestableEditingMenuBuilder e) {
		#[
			e.deleteAction,
			e.cutAction,
			e.copyAction,
