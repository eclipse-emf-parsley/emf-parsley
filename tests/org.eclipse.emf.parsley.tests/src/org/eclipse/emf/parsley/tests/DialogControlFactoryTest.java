...
public class DialogControlFactoryTest extends AbstractControlFactoryTest {
	...
	public void testStringFeatureWithProposals() {
		...
		factory = createWithInjector(injector);
		Control control = createControl(factory, fixtures.getTestPackage().getClassForControls_StringFeature());
		assertTextEditable(control, true);
		assertText(control, "");
		classForControlsInstance.setStringFeature("Foo");
	...
	protected DialogControlFactory createAndInitializeFactory() {
		DialogControlFactory dialogControlFactory = new DialogControlFactory(getCompositeParameter(), getEObjectParameter(classForControlsInstance));
		injectMembers(dialogControlFactory);
		// shell must be visibile since we need to check visibility of some controls
		getShell().open();
		return dialogControlFactory;
	}

	protected DialogControlFactory createWithInjector(Injector injector) {
		new DialogControlFactory(getCompositeParameter(), getEObjectParameter(classForControlsInstance));
