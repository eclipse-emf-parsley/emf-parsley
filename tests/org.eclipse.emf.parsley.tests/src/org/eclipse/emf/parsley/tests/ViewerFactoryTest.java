...
public class ViewerFactoryTest extends AbstractViewerTest {
	...
	@Test
	public void testCreateTreeViewer() {
		treeViewer = viewerFactory.createTreeViewer(getShell(), SWT.MULTI | SWT.H_SCROLL | SWT.V_SCROLL | SWT.FULL_SELECTION);
	...
	@Test
	public void testCreateTableColumnLabelProvider() { // NOSONAR: just verify it doesn't throw an exception
		viewerFactory.createTableColumnLabelProvider(fixtures.getTestPackage().getClassForControls_BooleanObjectFeature());
	}

	@Test
	public void testCreateTableViewerComparator() { // NOSONAR: just verify it doesn't throw an exception
		viewerFactory.createTableViewerComparator(Arrays.asList(fixtures.getTestPackage().getClassForControls_BooleanObjectFeature()));
	}
}
